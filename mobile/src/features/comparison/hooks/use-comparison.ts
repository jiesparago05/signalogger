...
export function useComparison() {
  ...
  const [usedRadius, setUsedRadius] = useState(100);
  ...
  const compareLocation = useCallback(async (lng: number, lat: number) => {
    ...
    try {
      ...
      setUsedRadius(result.radius || 100);
      return result.data;
    } catch (err) {
  ...
  return { routeData, locationData, usedRadius, loading, compareRoute, compareLocation };
}
